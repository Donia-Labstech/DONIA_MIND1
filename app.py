...
# ==================== v4.0 INTEGRATED FUNCTIONS ====================

# 1. معالجة النصوص الرياضية (LaTeX) لضمان العرض الصحيح
def clean_latex(text: str) -> str:
    """تنظيف وتنسيق رموز LaTeX للمسائل الرياضية"""
    # إصلاح الهروب المزدوج لرموز الرياضيات
    text = re.sub(r'(?<!\\)\\(?!begin|end|\(|\)|\[|\]|\{|\})', r'\\\\', text)
    # التأكد من إحاطة المعادلات بالعلامات الصحيحة
    text = re.sub(r'(?<!\\)\$([^\$]+?)(?<!\\)\$', r'$\1$', text)
    return text

# 2. فئة FPDF المحسنة لدعم العربية (حل مشكلة الحروف المقطعة)
class ArabicFPDF(FPDF):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        # تأكد من وجود المجلد fonts وملف الخط Amiri-Regular.ttf
        ...
        if os.path.exists(font_path):
            ...
            # تنبيه في حال عدم وجود الخط لضمان عدم توقف البرنامج
            st.warning("خط Amiri غير موجود في مجلد fonts، سيتم استخدام الخط الافتراضي.")

    def _process_arabic(self, txt):
        return get_display(reshape(str(txt)))

    def cell(self, w, h=0, txt='', border=0, ln=0, align='', fill=False, link=''):
        txt = self._process_arabic(txt)
...
def generate_simple_pdf_fpdf(content: str, title: str, subtitle: str = "") -> bytes:
    pdf = ArabicFPDF()
    pdf.add_page()
    ...
    return pdf.output(dest='S') # إرجاع bytes مباشرة

# 3. محرك البحث (Tavily) لجلب بيانات المنهاج الجزائري المحدثة
def web_search(query: str, max_results: int = 3) -> str:
    try:
        ...
        results = [f"- {r.get('title')}: {r.get('content')[:250]}" for r in response.get('results', [])]
        return "\n".join(results)
    except Exception:
        return ""

# 4. المساعد الصوتي باستخدام Whisper
def audio_transcribe(audio_bytes: bytes) -> str:
...
# 5. الواجهة المحدثة للمساعد العائم
def render_floating_assistant_with_audio():
    from streamlit_mic_recorder import mic_recorder
    
    # استدعاء المساعد التقليدي أولاً
    # render_floating_assistant() 
    
    st.sidebar.markdown("---")
    st.sidebar.subheader("🎤 المساعد الذكي (صوتي)")
    ...
    if audio and audio.get('bytes'):
        with st.sidebar.spinner("جاري تحليل صوتك..."):
            text = audio_transcribe(audio['bytes'])
            if text:
                st.sidebar.info(f"أنت قلت: {text}")
                # هنا يتم ربط النص بدالة التوليد الخاصة بك
