...
COPYRIGHT_FOOTER_AR = "جميع حقوق الملكية محفوظة حصرياً لمختبر DONIA LABS TECH © 2026 — donialabstech.online"
...
WEBSITE_URL = os.getenv("DONIA_WEBSITE_URL", "https://donialabstech.online")
...
class ArabicFPDF(FPDF):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # ── PRIORITY 0: Bundled font shipped inside the repo ───────────
        # This is the ONLY tier that works with zero network access and
        # zero dependency on what's installed on the host OS. Always
        # checked first. Path: <app_dir>/fonts/Amiri-Regular.ttf
        _bundled_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "fonts")
        _bundled_reg = os.path.join(_bundled_dir, "Amiri-Regular.ttf")
        _bundled_bold = os.path.join(_bundled_dir, "Amiri-Bold.ttf")
        if os.path.exists(_bundled_reg) and os.path.getsize(_bundled_reg) > 100_000:
            try:
                self.add_font("Amiri", "", _bundled_reg)
                b_src = _bundled_bold if (os.path.exists(_bundled_bold)
                        and os.path.getsize(_bundled_bold) > 100_000) else _bundled_reg
        ...
        # ── PRIORITY 2: Download Amiri → /tmp (network fallback only) ─
...
def ensure_font_files():
    ...
    Skipped entirely if the font is already bundled in the repo (fonts/ folder),
    since that is the preferred, network-free source."""
    _bundled = os.path.join(os.path.dirname(os.path.abspath(__file__)), "fonts", "Amiri-Regular.ttf")
    if os.path.exists(_bundled) and os.path.getsize(_bundled) > 100_000:
        return
...
def _docx_set_complex_font(run, font_name: str = "Amiri"):
    """Force a specific font for complex-script (Arabic) runs via w:rFonts/w:cs.
    Without this, Word falls back to whatever default cs-font the machine has,
    which is usually fine on Windows but not guaranteed everywhere."""
...
def _docx_add_footer(doc):
    """Adds the DONIA LABS TECH copyright + website line at the end of a document.
    Shared across all DOCX generators so the brand line stays consistent and
    only needs updating in one place."""
    doc.add_paragraph()
    p = _docx_para(doc, COPYRIGHT_FOOTER_AR, size=9, align=WD_ALIGN_PARAGRAPH.CENTER)
    for run in p.runs:
...
.donia-logo-frame{
  ...
  background:radial-gradient(circle at 50% 35%,#0d1326 0%,#000000 75%);
  border-radius:18px;padding:.85rem;margin:.4rem 0 .9rem;
  box-shadow:0 6px 24px rgba(0,0,0,.35);
  border:1px solid rgba(255,255,255,.06);
}
.donia-logo-frame img{
  border-radius:12px;display:block;
}
.donia-website-link{
  display:block;text-align:center;
  font-family:'Montserrat',sans-serif;font-size:.78rem;font-weight:600;
  letter-spacing:.04em;color:#145a32!important;
  margin-top:.35rem;text-decoration:none!important;
}
.donia-website-link:hover{color:#27ae60!important;text-decoration:underline!important}
</style>
""", unsafe_allow_html=True)
...
with st.sidebar:
    ...
    if os.path.isfile(_logo_path):
        ...
        st.markdown(
            f'''<div class="donia-logo-frame">
                <img src="data:image/jpeg;base64,{_logo_b64}" width="180">
            </div>
            <a class="donia-website-link" href="{WEBSITE_URL}" target="_blank" rel="noopener noreferrer">
                🌐 {WEBSITE_URL.replace("https://", "").rstrip("/")}
            </a>''',
            unsafe_allow_html=True,
        )
    ...
    st.markdown(
        ...
        <div class="donia-social">
          <a href="{WEBSITE_URL}" target="_blank" rel="noopener noreferrer" title="الموقع الرسمي">🌐 الموقع</a>
